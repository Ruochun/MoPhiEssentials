...
struct CubFloat3Add {
    CUB_RUNTIME_FUNCTION __forceinline__ __host__ __device__ float3 operator()(const float3& a, const float3& b) const {
...
template <typename T>
struct CubOpAdd {
    CUB_RUNTIME_FUNCTION __forceinline__ __host__ __device__ T operator()(const T& a, const T& b) const {
        return a + b;
    }
};
...
inline void cubDEMSortByKeys(T1* d_keys_in,
    ...
    cub::DeviceRadixSort::SortPairs(d_scratch_space, cub_scratch_bytes, d_keys_in, d_keys_out, d_vals_in, d_vals_out, n,
                                    0, sizeof(T1) * MOPHI_BITS_PER_BYTE, this_stream);
    MOPHI_GPU_CALL(cudaStreamSynchronize(this_stream));
}
